...
**Date:** 2026-04-07 (updated from 2026-04-05)
**Version:** 0.8.0 on npm (published April 6, 2026)
...
Verify is a 26-gate verification pipeline for AI agent actions. ALL CLEAN — 0 dirty on core scenario set. 33,056 real agent PRs scanned with empirical failure rates published.

### Current State (April 7, 2026)

| Test | Result | Notes |
|------|--------|-------|
| Core self-test | **ALL CLEAN — 0 bugs** | ~2,800 core scenarios, 0 dirty. 681→0 achieved April 5. |
| Aspirational scenarios | ~2,400 tagged, excluded | Infrastructure/advanced patterns for future gates. |
| Supply chain (nightly) | 690+ new scenarios per cycle | Autonomous generation running on Lenovo. |
| Improve loop | Autonomous fixes accepted | Triage, context injection, bundle size fixes unblocked it. |
| Two nightly runners | GitHub Actions (pure) + Lenovo (live+Docker) | Both at 3 AM UTC. Both synced. |
| npm traction | 590 downloads/week, 363 cloners/14d | Zero marketing. Organic discovery via npm keywords. |
| AIDev-POP scan | **33,056 PRs scanned, 5 agent profiles** | 8.5% raw finding rate, 3.4% high-confidence. $0 cost. |
| GitHub Action | Built, tested on PR #50 | Multi-provider LLM. Not yet on marketplace. |
| Taxonomy | 668+ shapes (650 + 18 GC from scan) | 18 gate calibration shapes from real-world data. |

**Verdict:** Clean floor + empirical validation. First published structural failure rates across real agent PRs.

### What's Working
- **26 gates** — grounding, syntax, constraints, containment, filesystem, infrastructure, security, a11y, performance, staging, browser, HTTP, and 13 more. 354 unit tests, 21,342 assertions, 0 failures.
...
**The story:** "Verify manufactures trust by forcing reality checks, constraining repetition, and converting failure into future reliability."

Never lead with "26 gates." Lead with consequences. Show the before-state (no verify). Then the after (with verify). End with the audit artifact.

```bash
npx @sovereign-labs/verify demo                     # default: liar-agent
npx @sovereign-labs/verify demo --scenario=liar      # Demo E (lead)
npx @sovereign-labs/verify demo --scenario=world     # Demo A
npx @sovereign-labs/verify demo --scenario=drift     # Demo B
```

**Implementation:** `src/cli.ts` case 'demo'. Each scenario ~70 LOC. ~250 LOC total. Uses real `govern()` with real gates, real K5, real grounding. Terminal output is colorized, human-readable.

**Recording format:** VS Code full screen — editor top, terminal bottom. OBS screen capture. Live narration (conversational, not scripted corporate). Subtle background music. Quick cuts, no dead time. Text overlays on key moments. Three individual clips + one combined 60-second version.

---

#### Launch Sequence: E → A → B

Each demo answers three boardroom questions:
...
The universal trust break. Every person who's used an AI agent has felt this.

**WITHOUT verify (5 seconds):**
```
...
**Narration:** "The agent said it was done. Your pipeline believed it. The next stage started. The report didn't exist."
...
Attempt 1:
  Agent claims: "Report saved successfully."
  ✗ File reports/weekly.md does not exist.
    The agent claimed it wrote the file. It didn't.
  → Remembered: can't claim success without evidence.
...
Attempt 2:
  Agent actually creates reports/weekly.md with content.
  ✓ Converged in 2 attempts.

  Audit trail:
  ├ Attestation: VERIFIED — filesystem✓ syntax✓ containment✓ (13 checks)
  ├ Banned: "claim completion without file evidence"
  └ Downstream: safe to proceed — artifact confirmed
```

**Narration:** "Same agent, same goal. But now verify checks reality, not the agent's status message. The file doesn't exist? Blocked. Second attempt — it actually does the work. And that lie pattern? Permanently banned. It won't work again."

**Final line:** "Verify does not trust status messages. It checks reality."

---

#### Demo A: "Wrong World Model" — THE ENGINE

The agent planned against a world that doesn't exist. Shows K5 constraint learning — the search space shrinks in real time.
...
**Narration:** "The agent wrote valid CSS. It just targeted something that doesn't exist. The code deployed. The page didn't change. Nobody knew why."
...
Attempt 1:
  Agent uses selector .profile-nav
  ✗ That selector doesn't exist in your code.
    Reality: .hero, .card, .nav-link, .team-list, .badge
  → .profile-nav permanently banned. Search space narrowed.
...
Attempt 2:
  Agent uses .nav-link — exists in reality.
  ✓ Converged in 2 attempts.

  Audit trail:
  ├ Attestation: VERIFIED — grounding✓ syntax✓ containment✓ (13 checks)
  ├ Banned: .profile-nav (selector does not exist in source)
  ├ Constraint: future attempts cannot reference .profile-nav
  └ K5 store: 1 constraint active — search space reduced
```

**Narration:** "The agent didn't make a syntax error. It made a reality error — it referenced something that doesn't exist. Verify knows what's actually in your code. And that wrong selector? Permanently banned. The agent's search space just got smaller. That's not retry logic — that's learning."

**Final line:** "The agent planned against a world that doesn't exist. Verify forced it into the real one."

---

#### Demo B: "The Silent Drift" — THE CLOSER

The agent completed the task. The app still works. But the system silently drifted. This is the fear that opens wallets.

**WITHOUT verify (5 seconds):**
```
Agent: "Updated hero section as requested. ✓"
→ Hero background changed to navy. Looks correct.
→ Agent also modified .env config flag and auth middleware route.
→ App works. Tests pass. Change ships.
→ Auth breaks 3 days later. Nobody connects it to the hero change.
```

**Narration:** "The agent did what you asked. The hero looks great. But it also quietly changed your auth config. The app still works — for now. Three days later, logins fail. Nobody connects it to the CSS change."
...
Attempt 1:
  Agent edits server.js — changes hero background.
  Agent also modifies .env DEBUG flag and config.json auth settings.
  ✗ Your edit changed 3 files but only declared changes to 1.
    Declared: server.js (.hero background)
    Undeclared: .env (DEBUG flag), config.json (auth.enabled)
    These changes were not in your predicates.
  → Remembered: edits must match declarations.
...
Attempt 2:
  Agent edits only server.js — hero background only.
  ✓ Converged in 2 attempts.

  Audit trail:
  ├ Attestation: VERIFIED — containment✓ syntax✓ grounding✓ (13 checks)
  ├ Blocked: 2 undeclared mutations (.env, config.json)
  ├ Constraint: future edits must match predicate scope
  └ Blast radius: contained to declared surface only
```

**Narration:** "The hero change looked perfect. But verify caught two things the agent didn't declare — a config flag and an auth setting. Those would have shipped silently. We don't just verify whether the task succeeded. We verify whether the system stayed coherent."

**Final line:** "The most dangerous agent failures are the ones that look like success."

---

#### Sidelined (not in launch demos)

**Security demo:** Risks positioning verify as "another SAST tool." Hold for later.
**Multi-agent demo:** Too niche. Market is still scared of one agent.

---

#### The 60-Second Combined Video

**Script:**

**(0-5s)** Text on screen: "What happens when AI agents edit your code?"

**(5-10s)** WITHOUT verify montage — agent says "done" three times, each time something is wrong. Quick cuts. No narration.

**(10-15s)** Text: "What if every action had to prove itself?"

**(15-30s) Demo E condensed:** "The agent said it saved the report. It didn't. Verify checked reality. Second attempt — it actually did the work."

**(30-45s) Demo A condensed:** "The agent used a selector that doesn't exist. Verify knows what's real. That selector is permanently banned. The search space shrinks."

**(45-55s) Demo B condensed:** "The agent completed the task. But it also quietly changed auth config. Verify caught the undeclared drift."

**(55-60s)** End card: **"Your agent just got better."** `npm install @sovereign-labs/verify`

**Post on:** Twitter, LinkedIn, Reddit r/MachineLearning, r/LocalLLaMA, HackerNews.
...
**Setup:**
1. Build demo commands (`src/cli.ts` — ~250 LOC)
2. VS Code open with `fixtures/demo-app/server.js` visible
3. Terminal panel at bottom
4. OBS recording at 1080p

**Per demo (3 takes each):**
1. Run the "without verify" version — just show the bad output
2. Run `npx @sovereign-labs/verify demo --scenario={name}`
3. Narrate live: conversational, like showing a coworker
4. Pause 2 seconds between sections for edit points

**Post-production:**
1. Trim dead space
2. Add text overlays: the ✗ and ✓ lines, the "why your stack missed this" callout
3. Add subtle tech-ambient background music
4. Export: 3 individual clips (30s each) + 1 combined (60s)
5. Thumbnail: before/after split image with tagline

**Time estimate:** 2 hours to build demo commands. 1 hour to record. 2 hours to edit. Ship same day.

---

#### The Full Trust Arc

| Stage | Format | What they feel | The line |
|-------|--------|----------------|----------|
| **Hook** | 60-second video (E→A→B) | "Oh no, that's happened to me" | "Your agent just got better." |
| **Trust** | `npx @sovereign-labs/verify self-test` | Proof | "You don't have to believe us. Run it." |
| **Need** | `npx @sovereign-labs/verify demo` | Need | Run it yourself, see convergence live. |
| **Awe** | Blog post / nightly report | Surprise | "It got smarter while you slept." |
| **Vision** | Paper / 90-day evidence | Wonder | "It found something nobody knew to look for." |

The video hooks them. Self-test proves it. The demo command lets them feel it. The blog and paper close the deal over time.

#### Verification
```bash
npm info @sovereign-labs/verify version  # Should show 0.6.x
npx @sovereign-labs/verify self-test     # Should work for any consumer
npx @sovereign-labs/verify demo          # Default: liar agent
npx @sovereign-labs/verify demo --scenario=liar       # Demo E
npx @sovereign-labs/verify demo --scenario=world      # Demo A
npx @sovereign-labs/verify demo --scenario=drift      # Demo B
```
...
1. **Audit `attestation` strings** across all 26 gates — rewrite to plain English. No gate codes in the user-facing summary. "Grounding gate failed: selector not found" → "That CSS selector doesn't exist in your code."
...
| Component | Changes? |
|-----------|----------|
| Gate sequence (26 gates + action gate) | **No** |
| K5 constraints | **No** |
| G5 containment | **No** |
| Narrowing | **No** |
...
Demo specs, sequencing, and trust arc are in **P3** (line ~275). Single source of truth for all demo work.
